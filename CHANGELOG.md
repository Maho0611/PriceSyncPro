...
- **修复带日期戳/带点号前缀的官方模型名被清理层错配（真实计费错误）**：对官方价格表全部 193 个含 4 位数字段的 bare key 做穷举审计，发现 66 个 key 无法精确匹配自身——匹配层在查裸表之前先做了结构性清理（剥离尾部日期戳、点号前缀），导致 `gpt-4o-2024-05-13`（$5/M）被错配到 `gpt-4o`（$2.5/M）、`mistral-large-2402`（$8/M）被错配到 `mistral-large`（$2/M）等 24 对价格不同的"带戳/基础版"模型同步了错误价格；`cohere.command-a-reasoning-08-2025`、`ft:gpt-4.1-2025-04-14` 这类 key 更是完全无法命中。现在裸表精确匹配（原始名 / 斜杠尾段）提前到所有清理层之前，渠道名与官方 key 完全一致时必定原样命中。审计脚本验证 193 个 key 全部精确自匹配，回归测试全过。
...
- **兜底候选下拉框完全不可见**：表格单元格默认的 `nowrap + overflow:hidden` 会把下拉框挤成一个省略号，用户完全看不出该处可以点击。现在带候选的单元格解除截断，下拉框改为整行宽的"按钮感"控件（橙色边框 + 浅橙底 + hover 反馈），占位文案改为"👉 点击选择兜底价格（N 个候选）"。
- **自动兜底开关改为默认打开**：仅当用户显式关闭过（存储了 false）才保持关闭。
